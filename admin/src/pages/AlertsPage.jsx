...
const AlertsPage = () => {
  ...
  useEffect(() => {
    fetchAlerts();
    const intervalId = setInterval(fetchAlerts, 30000); // Auto-refresh
  ...
  const updateAlertStatus = async (alertId, payload) => {
    ...
    try {
      ...
      await fetchAlerts(); // Re-fetch alerts to get the latest state
      return true;
    } catch (error) {
      ...
      <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <AlertTriangle
              className={`w-6 h-6 ${
                getStatusClass(alert.status).split(" ")[1]
              }`}
            />
            <h3 className="text-lg font-semibold text-gray-900">
              {alert.category} Alert - {alert.userId.slice(-6)}
            </h3>
            <span
            ...
            {alert.adminResponse && (
              <div className="p-4 bg-blue-50 rounded-lg">
                <h5 className="font-medium text-blue-900 mb-2">
                  Previous Response by {alert.assignedTo || "Admin"}:
                </h5>
            ...
            {alert.status !== "resolved" && (
              <div className="space-y-4">
                <textarea
                  ...
                  className="w-full p-3 bg-gray-50 border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
                  rows="4"
                />
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full sm:w-64 pl-11 pr-4 py-2.5 bg-gray-50 border-transparent rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
            />
          </div>
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="px-4 py-2.5 bg-gray-50 border-transparent rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
          >
      ...
      <main>
        {loading ? (
          <div className="flex justify-center items-center h-64">
            ...
            <p className="ml-2 text-gray-600">Loading alerts...</p>
          </div>
        ) : error ? (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex-shrink-0 lg:ml-6">
                      <button
                        ...
                        className="flex items-center justify-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg shadow-sm hover:bg-blue-700 transition-colors"
                      >
